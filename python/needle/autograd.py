...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        node_out_grad = sum_node_list(node_to_output_grads_list[node])
        node.grad = node_out_grad

        if node.op:
            grad_inputs = node.op.gradient_as_tuple(node_out_grad,node)
            for input_node, grad in zip(node.inputs, grad_inputs):
...
def topo_sort_dfs(node : Value, visited, topo_order: List[Value]):
    """Post-order DFS"""
    if node in visited:
        return
    visited.add(node)
